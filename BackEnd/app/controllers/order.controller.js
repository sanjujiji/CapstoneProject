...
exports.addOrders = (req,res) => {
    ...
    */
        ...
        products.find(filter,(err,results) => {
            if (results.length > 0){
                ...
                if ( productObj.availableItems < orderQuantity ){
                    // res.status(status.StatusCodes.OK).send({message : message});
                    reject();
                    // return;
                }
                else {
    ...
    promise
                        ...
                        .then(() => {
                            ...
                            promiseOrder.then(() => {
                                ...
                                orderObj.
                                    ...
                                    .catch((err) => {
                                        message = "Unable to find the maximum order Id";
                                        res.status(status.StatusCodes.INTERNAL_SERVER_ERROR).send({message : message});
                                        return;
                                    })
                            })
        ...
        .catch(() => {
            //This would throw the error if no product Id was found or if the stock was below the required quantity
            res.status(status.StatusCodes.BAD_REQUEST).send({message : message});
            return;
        })
}